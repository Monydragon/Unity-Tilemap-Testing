...
namespace Cinemachine
{
    ...
    public class CinemachineFramingTransposer : CinemachineComponentBase
    {
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            if (deltaTime >= 0)
                cameraOffset.z = Damper.Damp(cameraOffset.z, m_ZDamping, deltaTime);
            ...
            if (deltaTime < 0)
            {
                ...
                cameraOffset += OrthoOffsetToScreenBounds(targetPos, rect, -1, out moved);
            }
            else
            {
                ...
                cameraOffset += OrthoOffsetToScreenBounds(targetPos, hardGuideOrtho, -1, out moved);
                targetPos.x -= cameraOffset.x;
                targetPos.y -= cameraOffset.y;

                // Now move it through the soft zone, with damping
                if (!moved)
                    cameraOffset += OrthoOffsetToScreenBounds(targetPos, softGuideOrtho, deltaTime, out moved);
            }
