...
class Game_Map
  ...
  def check_passage(x, y, bit)
        all_tiles(x, y).each do |tile_id|
          ...
          if flag & 0x10 != 0                              # [Åô]: No effect on passage
                next             if flag & bit == 0      # [Åõ] : Passable but star
                return false if flag & bit == bit   # [Å~] : Impassable
          else
                return true  if flag & bit == 0  # [Åõ] : Passable
                return false if flag & bit == bit   # [Å~] : Impassable
          end
        end
